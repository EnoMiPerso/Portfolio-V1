...
1. **Repository root** must contain only: the **10** `.html` files, **`.nojekyll`**, and **one** folder named **`assets/`** (plus optional docs like `README.md`).
2. **Never** put `fms`, `platform`, `studio`, `rpg`, `ai-annotations`, `smart-inputs`, or `transformation-hub` at the **same level** as `assets/`. Those names must be **subfolders inside** `assets/`. If they sit next to `assets/`, every image path breaks.
3. **Upload order (web UI, fewer than 100 files per commit):**  
   - Create **`assets`**, open it → upload all files for **`assets/fms/`** → commit.  
   - Repeat inside **`assets`** for: `transformation-hub/`, `smart-inputs/`, `ai-annotations/`, `studio/`, `platform/`, `rpg/`.  
   - Then upload **root** files: all `.html` + `.nojekyll` in one batch.
4. **Checklist file:** open **`REPO-FILE-LAYOUT.txt`** in this project for the same tree in plain text while you upload.
